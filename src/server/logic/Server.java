...
public class Server implements ActionListener, InfoHandler {
        ...
        if (info.startsWith("prs")) {
            if (info.endsWith("1")) {
                ...
                if (!isInSameDirection(
                        getDirectionFromChar(info.charAt(3)), hero_1.getFacingStatus())) {
                ...
                // todo check if illegal
                if (!isInSameDirection(
                        getDirectionFromChar(info.charAt(3)), hero_2.getFacingStatus())) {
    ...
    private boolean isBlockedInDirection(Tank tank, int direction) {
        ...
        switch (direction) {
            case kDirectionLeft: {
                order_1 =
                    getOrderFromLocation(tankLocationX - tank.getMoveVelocity(), tankLocationY);
                order_2 = getOrderFromLocation(
                    tankLocationX - tank.getMoveVelocity(), tankLocationY + 16);
                break;
            }
            case kDirectionRight: {
                order_1 = getOrderFromLocation(
                    tankLocationX + 32 + tank.getMoveVelocity(), tankLocationY);
                order_2 = getOrderFromLocation(
                    tankLocationX + 32 + tank.getMoveVelocity(), tankLocationY + 16);
                break;
            }
            case kDirectionUp: {
                order_1 =
                    getOrderFromLocation(tankLocationX, tankLocationY - tank.getMoveVelocity());
                order_2 = getOrderFromLocation(
                    tankLocationX + 16, tankLocationY - tank.getMoveVelocity());
                break;
            }
            case kDirectionDown: {
                order_1 = getOrderFromLocation(
                    tankLocationX, tankLocationY + 32 + tank.getMoveVelocity());
                order_2 = getOrderFromLocation(
                    tankLocationX + 16, tankLocationY + 32 + tank.getMoveVelocity());
                break;
            }
        }
        ...
        synchronized (tanks) {
            for (Tank tankIter : tanks) {
                if (tank != tankIter) {
                    if (isTileBlockedByTank(order_1, tankIter)
                        || isTileBlockedByTank(order_2, tankIter)) {
                        return true;
                    }
                }
            }
        ...
        System.out.printf("moving%d %d %d %d %d\n",
            ...
            order_2.getKey(),
            order_2.getValue());
        if (tiles[order_1.getKey()][order_1.getValue()].isTankThrough()
            && tiles[order_2.getKey()][order_2.getValue()].isTankThrough()) {
    ...
    private boolean isTileBlockedByTank(Pair<Integer, Integer> order, Tank tank) {
        ...
        if ((locationX % 16 == 0) && (locationY % 16 == 0)) {
            if ((row >= blockLU.getKey()) && (row <= blockLU.getKey() + 1)
                && (column >= blockLU.getValue())
                && (column <= blockLU.getValue() + 1)) {
                return true;
            }
        }

        if ((locationX % 16 == 0) && (locationY % 16 != 0)) {
            if ((row >= blockLU.getKey()) && (row <= blockLU.getKey() + 2)
                && (column >= blockLU.getValue())
                && (column <= blockLU.getValue() + 1)) {
                return true;
            }
    ...
    private Pair<Integer, Pair<Pair<Integer, Integer>, Pair<Integer, Integer>>>
    getHittingWallInfoPack(Bullet bullet) {
        ...
        switch (bulletDirection) {
            case kMovingLeft: {
                order_1 = getOrderFromLocation(
                    bulletLocationX - bullet.getBulletVelocity(), bulletLocationY);
                order_2 = getOrderFromLocation(
                    bulletLocationX - bullet.getBulletVelocity(), bulletLocationY - 16);
                break;
            }
            case kMovingRight: {
                order_1 = getOrderFromLocation(
            ...
            case kMovingUp: {
                order_1 = getOrderFromLocation(
                    bulletLocationX - 16, bulletLocationY - bullet.getBulletVelocity());
                order_2 = getOrderFromLocation(
                    bulletLocationX, bulletLocationY - bullet.getBulletVelocity());
                break;
            }
            case kMovingDown: {
                order_1 = getOrderFromLocation(
        ...
        if (tiles[order_1.getKey()][order_1.getValue()].isBulletThrough()
            && tiles[order_2.getKey()][order_2.getValue()].isBulletThrough()) {
    ...
    private void destroyTile(Pair<Integer, Integer> pairRowColumn) {
        if (tiles[pairRowColumn.getKey()][pairRowColumn.getValue()].isDamageable()) {
            if (tiles[pairRowColumn.getKey()][pairRowColumn.getValue()].isDecisive()) {
                ...
                tiles[pairRowColumn.getKey()][pairRowColumn.getValue()] =
                    new PlainTile(pairRowColumn.getKey(), pairRowColumn.getValue());
                broadcast(
                    String.format("det_%d_%d", pairRowColumn.getKey(), pairRowColumn.getValue()));
            }
        }
    }
    ...
    private void correctTankLocation(Tank tank, int direction) {
        switch (direction) {
            ...
            case kDirectionRight: {
                int locationX = tank.getLocationX();
                int upper =
                    (locationX / kTankPositionCorrectionUnit + 1) * kTankPositionCorrectionUnit;
            ...
            case kDirectionDown: {
                int locationY = tank.getLocationY();
                int upper =
                    (locationY / kTankPositionCorrectionUnit + 1) * kTankPositionCorrectionUnit;
    ...
    private void updateStatus() {
        ...
        synchronized (bullets) {
            ...
            while (bulletIterator.hasNext()) {
                ...
                Pair<Integer, Pair<Pair<Integer, Integer>, Pair<Integer, Integer>>>
                    hittingInfoPack = getHittingWallInfoPack(bullet);
        ...
        synchronized (bullets) {
            for (int i = 0; i < bullets.size(); i++) {
                for (int j = i + 1; j < bullets.size(); j++) {
                    if (isBulletsReducible(bullets.get(i), bullets.get(j))
                        || isBulletsReducible(bullets.get(j), bullets.get(i))) {
    ...
    private boolean isBulletHittingTank(Bullet bullet, Tank tank) {
        ...
        if ((tankLocationX % 16 == 0) && (tankLocationY % 16 == 0)) {
            ...
            switch (bullet.getVelocityStatus()) {
                case kMovingLeft:
                case kMovingRight:
                    if ((bulletLocationY == tankCentLocationY)
                        && (Math.abs(bulletLocationX - tankCentLocationX) <= 2)) {
                        return true;
                    }
                    break;
                case kMovingUp:
                case kMovingDown:
                    if ((bulletLocationX == tankCentLocationX)
                        && (Math.abs(bulletLocationY - tankCentLocationY) <= 2)) {
                        return true;
                    }
                    break;
            }
            ...
            switch (bullet.getVelocityStatus()) {
                ...
                case kMovingUp:
                case kMovingDown:
                    if ((bulletLocationX == tankLineLocationX_1)
                        || (bulletLocationX == tankLineLocationX_2)) {
            ...
            switch (bullet.getVelocityStatus()) {
                case kMovingLeft:
                case kMovingRight:
                    if ((bulletLocationY == tankLineLocationY_1)
                        || (bulletLocationY == tankLineLocationY_2)) {
    ...
    private boolean isInBoarder(Pair<Integer, Integer> pair) {
        return (pair.getKey() >= 0 && pair.getKey() < MAX_MAP_SIZE && pair.getValue() >= 0
                && pair.getValue() < MAX_MAP_SIZE);
    }
