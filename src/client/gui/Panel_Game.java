...
public class Panel_Game extends JPanel {
    ...
    public void paint(Graphics graphics) {
        ...
        // paint layer -1
        graphics.drawImage(
            new ImageIcon(getClass().getResource("/res/pic/plain_tile.png")).getImage(),
            0,
            0,
            480,
            480,
        ...
        for (int i = 0; i < 30; i++) {
            for (int j = 0; j < 30; j++) {
                if (tiles[i][j].getPaintLayer() == 0) {
                    graphics.drawImage(tiles[i][j].getImage(),
                        tiles[i][j].getPositionX(),
                        tiles[i][j].getPositionY(),
                        this);
                }
            }
        }
        ...
        if (hero_1.isActivated()) {
            graphics.drawImage(
                hero_1.getImage(), hero_1.getLocationX(), hero_1.getLocationY(), this);
        }

        if (hero_2.isActivated()) {
            graphics.drawImage(
                hero_2.getImage(), hero_2.getLocationX(), hero_2.getLocationY(), this);
        }
        ...
        synchronized (bullets) {
            for (Bullet bullet : bullets) {
                graphics.drawImage(
                    bullet.getImage(), bullet.getLocationX() - 2, bullet.getLocationY() - 2, this);
            }
        }
